...
\subsection{Mesh}
(ANS 2013W pre-dated the Mesh class, so we need to talk about this)
...
Mesh Oriented datABase (MOAB) \cite{tautges\_moab:\_2004}
PyTAPS the Python interface to ITAPS \cite{pytaps}
...
\cite{tautges\_acceleration\_2009}.  A \texttt{dagmc} module has been added to
...
to seamlessly create PyNE \texttt{Mesh} object tagged with materials, where the
materials are mixtures of the contributions from the various geometry cells
found in each mesh volume element. This functionilty is especially 


\subsection{R2S Activation Workflow}

The Rigorous Two-Step (R2S) method {chen\_rigorous\_2002}

In memory! leverage the 
PyNE \texttt{mcnp}, \texttt{mesh},
\texttt{material}, \texttt{alara}, \texttt{dagmc}, and \texttt{sampling}


ALARA \cite{wilson\_validation\_1998}
